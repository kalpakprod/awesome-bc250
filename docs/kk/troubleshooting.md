...
| MangoHud **655 %** GPU қолданысын көрсетеді | amdgpu белсенділік метрикасын `0xFFFF`-те қалдырады; MangoHud 65535/100 оқиды | cyan-skillfish-governor-smu (smu тармағы) іске қосыңыз — ол `gpu_metrics`-ті патчтайды; MangoHud-ты өзгертудің қажеті жоқ. Немесе жеке **`install_gpu_usage_fix.sh`** скриптін қолданыңыз ([Old Lamer — Part XV](https://youtu.be/lSipaWjU6D4)) ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/performance.md)) → [09 — Оверклок](../en/09-overclock-undervolt.md) |
...
| Ойын **OOM крэш / артефакттар содан кейін өледі** (RDR2, CoH3) | **512 MB динамикалық VRAM + ZRAM** қақтығысы немесе жай **RAM жетіспеуі** | BIOS-ты **бекітілген VRAM**-ға ауыстырыңыз (мыс. 10 GB RAM / 6 GB VRAM); **немесе** systemd ZRAM-ды өшіріп, **zswap + 32 GB Btrfs swapfile** пайдаланыңыз ([Old Lamer — Part XIV](https://youtu.be/A6juAoY70aU), рецепт [06](../en/06-linux.md)/[09](../en/09-overclock-undervolt.md)) ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/stability.md)) → [08 — BIOS](../en/08-bios.md) |
...
| Интернет-провайдер **Steam-ді** баяулатады | Steam CDN трафигіне DPI/троттлинг | Троттлингке қарсы құралдар (`zapret`-тәрізді) көмектеседі — бірақ **Bazzite-тің тек оқуға арналған ФЖ оларды бөгейді**; өзгертілетін дистрибутив (Fedora/Arch) пайдаланыңыз. RU-оператор ерекшеліктері (Yota, zapret+warp) [орысша нұсқада](../ru/06-linux.md) → [06 — Linux](../en/06-linux.md) |
...
- [4pda BC-250 тақырыбы](https://4pda.to/forum/index.php?showtopic=1104980) — RU интернет-провайдер Steam-троттлингі (Yota, zapret+warp).

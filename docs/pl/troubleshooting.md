...
| MangoHud pokazuje **655%** użycia GPU | amdgpu pozostawia metrykę aktywności na `0xFFFF`; MangoHud odczytuje 65535/100 | Uruchom cyan-skillfish-governor-smu (gałąź smu) — łata `gpu_metrics`; nie trzeba zmieniać MangoHud ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/performance.md)) → [09 — Podkręcanie](../en/09-overclock-undervolt.md) |
...
| Gra **crashuje przez OOM / artefakty, potem pada** (RDR2, CoH3) | Konflikt **512 MB dynamicznego VRAM + ZRAM** | Przełącz BIOS na **stały VRAM** (np. 10 GB RAM / 6 GB VRAM) albo wyłącz ZRAM ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/stability.md)) → [08 — BIOS](../en/08-bios.md) |

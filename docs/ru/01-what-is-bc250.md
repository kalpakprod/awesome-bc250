...
BC-250 — не единственный у AMD «остаточный консольный кремний на настольной плате». У неё два близких родственника, построенных по той же идее: **AMD 4700S Desktop Kit** (CPU-кит на базе **PlayStation 5**) — чат предупреждает, что его перепутанно выставляют рядом с BC-250 на маркетплейсах ([02-buying.md](02-buying.md)) — и **AMD 4800S Desktop Kit**, версия на базе **Xbox Series X** (8 ядер Zen 2, заведённых на GDDR6, с отключённым GPU RDNA 2 от консоли). Оба — реальные продукты AMD, которые, как и BC-250, сочетают спасённый консольный CPU с распаянной GDDR6 ([VideoCardz](https://videocardz.com/newz/amd-4800s-desktop-kit-a-pc-repurposed-apu-from-xbox-series-x-has-been-tested)). Это полезный контекст, чтобы отличать BC-250 от её родственников при покупке.
...
- ⚠ **eGPU её не спасёт — сообщает сообщество (r/BC250Gaming).** Единственный слот M.2 — это всего лишь **PCIe 2.0 ×2** (см. карточку по железу ниже), и на такой пропускной способности внешняя видеокарта, подключённая через M.2, **по отзывам работает *хуже*, чем встроенный GPU RDNA 2**: медленная шина её душит. Если нужна бóльшая графическая мощность — консенсус в том, что это не та плата. *(Сообщает сообщество; считайте предостережением, а не бенчмарком.)*

> ⚠ **Что означает двухцветный светодиод — сообщает сообщество (r/BC250Gaming).** Двухцветный светодиод рядом с сетевой картой — это **майнинговый индикатор загрузки, а не лампа ошибки**: по отзывам сообщества **красный = GPU/ОЗУ *не* загружены на 100 %, зелёный = полная загрузка**. То есть красный на простаивающей настольной плате — это норма, а не неисправность. *(Сообщает сообщество; AMD не выпускает документацию на эту плату, поэтому точное соответствие цветов считайте неподтверждённым.)*
...
Характеристики сверены с реверс-инжинирингом железа сообществом (AMD даташит не публикует). Параметры шины памяти и физические габариты, ранее неподтверждённые, теперь взяты из [спецификации железа elektricM](https://github.com/elektricm/elektricm) (которая ссылается на mothenjoyer69 / Segfault / neggles / yeyus за реверс-инжиниринг). Распиновка и цифры питания ниже взяты из канонического документа сообщества по железу.
...
| GPU | **24 вычислительных блока, RDNA 2** (`gfx1013`; у SoC PS5 — 36); растеризация ≈ **между RX 6600 и RX 6600 XT** / уровень GTX 1660 Ti; **Vulkan 1.4** | [hardware.md](https://github.com/mothenjoyer69/bc250-documentation) · чат ([src](https://t.me/c/2424231195/11282)) · [elektricM](https://github.com/elektricm/elektricm) |
...
| Шина / пропускная способность памяти | **256-битная** GDDR6 @ **14 Гбит/с**, **~448 ГБ/с** | [спецификация железа elektricM](https://github.com/elektricm/elektricm) |
...
| Видеовыход | **DisplayPort 1.4** (до **4K@120 / 8K@60**); для HDMI — переходник DP→HDMI; несёт звук | ([src](https://t.me/c/2424231195/9148)) · [elektricM](https://github.com/elektricm/elektricm) |
| Накопитель (M.2) | 1x M.2 2280 — **PCIe 2.0 x2 или SATA III** | [elektricM](https://github.com/elektricm/elektricm) |
| 2-й DisplayPort | присутствует, но **не распаян**; активируется программно | ([src](https://t.me/c/2424231195/88026)) |
| Физический размер | длина **340 мм / 310 мм** (в зависимости от метода измерения), ширина **~115 мм**, **~400 г** с радиатором; нестандартный майнинговый форм-фактор | [спецификация железа elektricM](https://github.com/elektricm/elektricm) |

> ⚠ **Разгон GDDR6 = пропускная способность, а не FPS — сообщает сообщество (r/BC250Gaming).** По отзывам сообщества, разгон GDDR6 поднимает пропускную способность памяти примерно с **~256 ГБ/с до ~445 ГБ/с**, но **прироста в играх не даёт** — узкое место это 24 CU самого GPU, а не пропускная способность памяти, так что лишняя полоса в играх не используется. (Учтите: проверенная *стоковая* цифра выше — уже **~448 ГБ/с** при 256 битах / 14 Гбит/с, так что «база ~256 ГБ/с» из отзывов не сходится с паспортной — точные цифры ГБ/с считайте неподтверждёнными; устойчивая часть вывода в том, что FPS вы не получите.) О разгоне GPU/памяти в целом см. [09-overclock-undervolt.md](09-overclock-undervolt.md).

> **О габаритах платы:** [спецификация железа elektricM](https://github.com/elektricm/elektricm) приводит длину **340 мм / 310 мм** (две цифры отражают разные методы измерения), ширину **~115 мм** и **~400 г** с радиатором, на нестандартном майнинговом форм-факторе. Сам канонический `hardware.md` размеры не приводит; самый залайканный «железный» пост чата так и называется — *«Размеры amd bc-250»* (❤20 — [src](https://t.me/c/2424231195/379)), что подтверждает: для сборки корпусов это людям важно. Для точной подгонки корпуса работайте по измеренной 3D-модели — каталогизированные сообществом STL платы (например, `BC250 Board.stl`, [Printables 1103626](https://www.printables.com/model/1103626-amd-bc250-board) и точная модель на [Printables 1341336](https://www.printables.com/model/1341336-accurate-3d-model-of-the-amd-bc-250-board)) геометрически верны. См. [05-case.md](05-case.md).
...
- Это разъёмы **Molex Micro-Fit BMI** ([деталь 444280801](https://www.molex.com/en-us/products/part-detail/444280801)), а *не* штекеры PCIe/EPS — они питали плату внутри её исходного майнингового шасси. **J2000 и J2001 не одинаковы:** как видно из распиновки выше, на J2000 идут пины **LED1/LED2**, а на J2001 — пин **PGD**, так что разъёмы различаются ([документация по железу elektricM / mothenjoyer69](https://github.com/mothenjoyer69/bc250-documentation)).
- **PGD** (на J2001) — пин power-good/сигнальный: он видит **5 В, когда плата установлена в PSU2 стеллажа**. В автономной сборке вы обычно питаетесь через J1000, а J2000/J2001 можно игнорировать — но сверьтесь с [03-power-supply.md](03-power-supply.md) под ваш конкретный переходник БП.
...
- Шина/пропускная способность памяти, физические габариты, позиционирование GPU, DP 1.4, M.2 — [спецификация железа elektricM](https://github.com/elektricm/elektricm) (ссылается на mothenjoyer69 / Segfault / neggles / yeyus за реверс-инжиниринг)
...
- Родственные киты — AMD 4700S (CPU-кит PS5, выставляют рядом с BC-250, [02-buying.md](02-buying.md)) и AMD 4800S (CPU Xbox Series X + GDDR6, GPU отключён) — [VideoCardz: 4800S Desktop Kit](https://videocardz.com/newz/amd-4800s-desktop-kit-a-pc-repurposed-apu-from-xbox-series-x-has-been-tested)
- eGPU через M.2 медленнее встроенного GPU (M.2 это PCIe 2.0 ×2), двухцветный светодиод у сетевой = индикатор загрузки (красный = не 100 % загрузки, зелёный = полная), разгон GDDR6 поднимает пропускную способность (~256→~445 ГБ/с) без прироста в играх — сообщает сообщество (r/BC250Gaming)

> AMD не публикует первичный даташит на эту плату; цифры выше — это лучший реверс-инжиниринг сообщества (канонический `hardware.md` плюс спецификация железа elektricM). Правки приветствуются через PR (см. [CONTRIBUTING.md](../../CONTRIBUTING.md)).

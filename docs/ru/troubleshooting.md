...
| Чёрный экран на **рабочем столе Bazzite KDE/Plasma** (KWin не может вывести картинку) | Проверка Atomic Mode Setting в KWin опирается на DTM TA, который APU BC-250 не инициализирует (по отзыву сообщества, [r/BC250Gaming]) | Задай **`KWIN_DRM_NO_AMS=1`** в drop-in `kwin-drm.conf` (`~/.config/environment.d/kwin-drm.conf`), затем перезайди ([ublue-os/bazzite #4447](https://github.com/ublue-os/bazzite/issues/4447)) → [06 — Linux](06-linux.md) |
...
| MangoHud показывает **655 %** загрузки GPU | amdgpu оставляет метрику активности `0xFFFF`; MangoHud читает 65535/100 | Запусти cyan-skillfish-governor-smu (ветка smu) — он патчит `gpu_metrics`; настройка MangoHud не нужна. Либо примени отдельный скрипт **`install_gpu_usage_fix.sh`** ([Old Lamer — Part XV](https://youtu.be/lSipaWjU6D4)) ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/performance.md)) → [09 — Разгон](09-overclock-undervolt.md) |
...
| Игра **крашится OOM / артефакты, потом вылет** (RDR2, CoH3) | Конфликт **512 МБ динамической VRAM + ZRAM** или просто **нехватка RAM** | Переключи BIOS на **фиксированную VRAM** (напр. 10 ГБ RAM / 6 ГБ VRAM); **либо** отключи systemd ZRAM и используй **zswap + 32 ГБ Btrfs-swapfile** ([Old Lamer — Part XIV](https://youtu.be/A6juAoY70aU), рецепт в [06](06-linux.md)/[09](09-overclock-undervolt.md)) ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/stability.md)) → [08 — BIOS](08-bios.md) |
...
| Оператор **режет скорость Steam** | DPI/троттлинг трафика Steam CDN | Анти-троттлинг (`zapret` и подобные) помогает — но **read-only-ФС Bazzite его блокирует**, бери изменяемый дистрибутив (Fedora/Arch). На **Yota** Steam падает до ~2.5 МБ/с; на Windows связка **zapret+warp** возвращает 10–20 МБ/с ([4pda](https://4pda.to/forum/index.php?showtopic=1104980)) → [06 — Linux](06-linux.md) |
...
| **Периодический no-POST на б/у плате** (иногда стартует, иногда мёртвая; не из-за прошивки) | ⚠ Сообщаемый сообществом режим отказа: **трещины в BGA-пайке от термоциклов** под APU (платы из-под майнинга, прошли много нагрев-циклов) — по отзыву сообщества (r/BC250Gaming), elektricM не подтверждает | Сначала исключи причины выше (питание/CMOS/тайминги RAM). **Только продвинутое / крайнее средство:** **реболл/рефлоу APU** (контролируемый прогрев, чтобы переусадить BGA-шарики) — фикс от сообщества; высокий риск убить плату, нужны фен/рефлоу-станция и навык; реболлинг надёжнее, если есть инструмент → [08 — BIOS](08-bios.md) |
...
- Old Lamer (YouTube): [Part XIV — zswap + 32 ГБ Btrfs-swap](https://youtu.be/A6juAoY70aU) · [Part XV — `install_gpu_usage_fix.sh`](https://youtu.be/lSipaWjU6D4)
- [Тред BC-250 на 4pda](https://4pda.to/forum/index.php?showtopic=1104980) — троттлинг Steam у российских операторов (Yota, zapret+warp).
- Чёрный экран Bazzite KDE / `KWIN_DRM_NO_AMS=1` — [ublue-os/bazzite #4447](https://github.com/ublue-os/bazzite/issues/4447).
- Периодический no-POST → трещины BGA / рефлоу APU как крайнее средство — по отзыву сообщества (r/BC250Gaming), у elektricM нет; общий механизм отказа BGA от термоциклов хорошо известен.

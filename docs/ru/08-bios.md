...
> ⚠ **verify — какой мод гонять, спорно.** В чате текущей/лучшей базой считают **мод 5.00**. Гайд комьюнити по прошивке держит **противоположную** позицию: он называет **модовый ROM chipset-меню P3.00 «стандартом комьюнити… самым стабильным и обкатанным»**, а **`P5.00_clv`** считает *только для экспертов* — он «открывает **Всё»** (каждое скрытое меню, включая экспериментальный **ReBAR / Resizable BAR** и debug/chipset-настройки), из-за чего его *«очень легко окирпичить, если поменять не то… Сиди на P3.00, если ты не продвинутый юзер»*. Важно: **`P5.00_clv` не выложен ни в одном публичном репозитории**, который смог найти гайд, — он ходит только как вложение в Discord, так что **канонического хеша для сверки нет**. Если хочешь его — совет гайда: взять копии у **двух** людей, гоняющих его независимо, и убедиться, что у обоих **одинаковый SHA-256** перед прошивкой. Ради разблокировки VRAM или chipset-настроек **хватает публично-проверенного ROM chipset-меню P3.00** ([elektricM: прошивка BIOS](https://elektricm.github.io/amd-bc250-docs/bios/flashing/)). Выбирай базу соответственно; если сомневаешься — P3.00.

...
BC-250 — собранная AsRock майнинг/серверная плата с урезанным PS5-APU «Oberon». Прошивка UEFI живёт на **16 МБ SPI-флеш-чипе** (Winbond **W25Q128** / Macronix MX25L128 в корпусе SOIC-8). Стоковая прошивка сильно залочена: в Setup почти ничего полезного не открыто. В чате встречаются стоковые версии **3.00**, **4.00** и **5.00**; моды пересобраны из них (номер версии — твой ориентир, всегда отмечай, на какой базе собран мод). Единственное функциональное отличие стоковых **v4.0** и **v5.0** — в том, что в v5.0 по умолчанию включена **сетевая загрузка (network boot)** ([src](https://discord.com/channels/1315924807128449065/1316030225338863636/1326825429595848816)).
...
> **Особенности мода 5.00, которые стоит знать.** В его Setup показана **частота CPU по умолчанию 3600** — это косметическое значение в UI, а не применённая частота ([src](https://discord.com/channels/1315924807128449065/1452152658168381593/1452406964780007515)). Также он открывает опцию **`x1x1x1x1` PCIe-бифуркации** в chipset-настройках ([src](https://discord.com/channels/1315924807128449065/1452152658168381593/1474231812598534351)). С таймингами памяти на этой базе будь особенно осторожен: **экстремальные значения таймингов могут окирпичить плату до внешней перепрошивки, и на P5.00 это проявляется сильнее** ([src](https://discord.com/channels/1315924807128449065/1371527281947971604/1468745940994101372)). И как при любой прошивке, переход на мод 5.00 может оставить **без картинки до сброса CMOS** ([src](https://discord.com/channels/1315924807128449065/1315933088668454942/1508568321346502892)).
...
> ★ **Почему 512 МБ UMA *прибавляют* FPS (механизм).** Выставление UMA-буфера в **512 МБ** не обделяет GPU — оно позволяет системе **динамически балансировать RAM и VRAM**, вместо того чтобы запирать большой фиксированный кусок, и именно этой перебалансировке приписали реальный скачок FPS: Cyberpunk 2077 поднялся с **60 → 66 fps (на разгоне 2 ГГц) → 76 fps** при FSR 3.0 *balanced*, 1080p, пресет Steam Deck ([Old Lamer — часть I](https://youtu.be/ohpEY2XQ_lo) ~11:21; ⚠ примерно — цифры сняты с видео, считай результатом одной сборки). Так что «512 МБ — лучше всего» — это не просто безопасный размер: маленький динамический буфер *сам по себе* часть истории про производительность, а не компромисс.
...
### Низкоуровневые разъёмы, отладка и чипы на плате

Кроме разъёма прошивки J4004 выше, на плате есть ещё несколько разъёмов и известный набор чипов. Они отреверсены в железных доках elektricM и полезны для сброса CMOS, отладочного зондирования, подключения вентиляторов и подтверждения, какой чип есть какой, перед прошивкой. Значения пинов перенесены дословно из ([elektricM: pinouts](https://elektricm.github.io/amd-bc250-docs/hardware/pinouts/)).

**CLRCMOS1 — джампер сброса CMOS (3-pin).** Это тот самый джампер, который во всей главе упоминается как «замкни перемычку CMOS» — вот его карта:

| Положение | Поведение |
|---|---|
| Пины 1–2 | CR2032 питает CMOS (по умолчанию) |
| Пины 2–3 | Сброс CMOS |

> 💡 Когда [чек-лист перед прошивкой](#перед-прошивкой--чек-лист-безопасности) и [«После каждой прошивки»](#после-каждой-прошивки--сбрось-cmos-не-пропускай) велят «замкнуть перемычку CMOS на ~20 секунд», **CLRCMOS1** — это она: переставь её с пинов 1–2 на пины 2–3, подожди, затем верни обратно. (Альтернатива — вынуть CR2032 на 60+ секунд.)

**TPMS1 — отладочный разъём LPC (18-pin, шаг 2.0 мм):**
...
| 9 | 3V | 10 | LAD1 |
| 11 | LAD0 | 12 | GND |
| 13 | (пусто) | 14 | S_PWRDWN# |
| 15 | 3VSB | 16 | SERIRQ# |
| 17 | GND | 18 | GND |

> 💡 **Пин 9 (3V) под напряжением только когда плата включена** — поэтому он работает как сигнал «система включена». Это делает его альтернативной точкой съёма для сборок с автозапуском / переходником под настоящий ATX (см. [джампер `AUTO_PWRON` в 03-power-supply.md](03-power-supply.md)).

**J2 — отладочный разъём JTAG/HDT (20-pin, шаг 1.27 мм, не распаян, на нижней стороне платы):**
...
> TEST18, TEST19 и DBRDY0 оставлены висящими. Это **единственный** аппаратный интерфейс сброса/отладки на плате.

**I2C_HEADER1 (3-pin):** `SCL · SDA · GND`. SCL — пин, **ближний к разъёмам питания**. Эта шина несёт **PMBUS к PMIC от Intersil** — точка доступа к телеметрии питания.

**CPU_FAN1 (4-pin):** `PWM · Tach · 12V · GND`.

**J4003 — разъём нескольких вентиляторов (16-pin, 2×8, 2.54 мм):**

| Ряд 1 | 1 GND | 2 F1T | 3 F2T | 4 F3T | 5 F4T | 6 F5T | 7 DET | 8 (пусто) |
|---|---|---|---|---|---|---|---|---|
| **Ряд 2** | 1 GND | 2 F1P | 3 F2P | 4 F3P | 5 F4P | 6 F5P | 7 GND | 8 GND |

Здесь `T` = tach (тахометр), `P` = PWM, для каждого вентилятора 1–5.

> 💡 **DET (ряд 1, пин 7) притянут к земле, когда плата стоит на плате вентиляторов / распределения питания** — то есть он детектирует несущую плату. (Перенумерация вентиляторов BIOS↔Linux разобрана в [06-linux.md](06-linux.md); здесь она не дублируется.)

**Чипы на плате (BOM).** В репозитории уже названы `SIO1_R` и `BIOS_A1` в разделах прошивки, но никогда не давались парт-номера и размеры; эта таблица позволяет прошивающему подтвердить, какой чип есть какой (16 МиБ Winbond — это BIOS, 512 КиБ Macronix — это SuperIO, его не трогай):

| Обозначение | Деталь | Роль |
|---|---|---|
| PUA1 | Intersil ISL69247 | Главный PMIC |
| PUIO1 | Intersil ISL95712 | PMIC питания ядер |
| PUA11… | Intersil ISL99360 | Smart power stages (фазы) |
| M2U2 | NXP CBTL04083B | Мультиплексор PCIe x4 2:1 |
| U30 | Realtek RTL8111H | Сетевой контроллер Ethernet (PCIe x1) |
| SU1 | AMD 218-0844029 | Чипсет FCH A68H «Bolton-D2H» |
| UIO1 | Nuvoton NCT6686D | SuperIO (чип датчиков hwmon) |
| BIOS_A1 | Winbond 25Q128JVSQ | 16 МиБ SPI-флеш = **BIOS** (шей ЭТОТ) |
| SIO1_R | Macronix MX25L4006E | 512 КиБ SPI-флеш = программа SuperIO (**НЕ шей — окирпичит SuperIO**) |

> Названный здесь чип датчиков SuperIO (Nuvoton **NCT6686D**) — это тот, к которому привязывается Linux-драйвер `nct6687`/`nct6683`; настройку датчиков/вентиляторов см. в [06-linux.md](06-linux.md).

**Инструменты для прошивки (продвинутое).** Две утилиты постоянно всплывают для копания в образе:

- **`psptool`** осматривает и извлекает блобы AMD-прошивки внутри дампа BIOS. `psptool -E bios.bin` выводит список записей; `psptool -X -d 0 -e 1 -o firmware.bin bios.bin` вытаскивает прошивку SMU для анализа. ([bc250-collective/amd_smu_reverse_engineering](https://github.com/bc250-collective/amd_smu_reverse_engineering))
- **`Zentool`** патчит микрокод CPU — например, чтобы заменить инструкцию `RDRAND`. ([AMD-BC-250/documentation #1](https://github.com/AMD-BC-250/documentation/issues/1))

---

## Secure Boot и CSM (предусловия загрузки)

Добавь эти два пункта в список предусловий настройки BIOS — иначе **кастомные/патченые ядра не загрузятся** (патч 40-CU, патч частоты и т.д.):
...
Источник: [elektricM: устранение проблем загрузки](https://elektricm.github.io/amd-bc250-docs/troubleshooting/boot/).
...
- ★ 512 МБ UMA → динамический баланс RAM/VRAM → механизм прироста FPS (Cyberpunk 60 → 66 на разгоне 2 ГГц → 76 fps, FSR 3.0 balanced, 1080p, пресет Steam Deck) — [Old Lamer — часть I](https://youtu.be/ohpEY2XQ_lo) ~11:21 (⚠ примерно, снято с видео)

...
| MangoHud **%655** GPU kullanımı gösteriyor | amdgpu, aktivite metriğini `0xFFFF`'te bırakır; MangoHud 65535/100 okur | cyan-skillfish-governor-smu çalıştırın (smu dalı) — `gpu_metrics`'i yamalar; MangoHud değişikliği gerekmez ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/performance.md)) → [09 — Overclock](../en/09-overclock-undervolt.md) |
...
| Oyun **OOM çöküyor / artefaktlar sonra ölüyor** (RDR2, CoH3) | **512 MB dinamik VRAM + ZRAM** çakışması | BIOS'u **sabit VRAM**'e geçirin (örn. 10 GB RAM / 6 GB VRAM) veya ZRAM'i devre dışı bırakın ([elektricM](https://github.com/elektricM/amd-bc250-docs/blob/main/docs/troubleshooting/stability.md)) → [08 — BIOS](../en/08-bios.md) |

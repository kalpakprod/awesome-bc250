...
## 📈 Что реально даёт тюнинг

Каждая крутилка на этой плате чем-то платит. Вот четыре главные — по замерам сообщества. Прочитай их до того, как что-то прошивать.

<p align="center">
  <img src="assets/diagrams/gpu-clock-tradeoff.svg" width="100%" alt="Частота ядра GPU: прирост FPS растёт примерно до 95% к 2000 МГц и дальше упирается в процессор, а нагрев и потребление продолжают расти до 95%. Оптимум около 2000 МГц, примерно +30% FPS.">
</p>

<p align="center">
  <img src="assets/diagrams/undervolt-tradeoff.svg" width="100%" alt="Андервольт: эффективность растёт с 20% до 95% при снижении напряжения с 1129 мВ до 700 мВ, а риск нестабильности растёт с 15% до 100%. Стабильный для игр диапазон примерно 905–955 мВ; ниже 700 мВ GPU фиксируется на 1500 МГц.">
</p>

<p align="center">
  <img src="assets/diagrams/cu40-tradeoff.svg" width="100%" alt="Разблокировка 40 CU: вычисления растут с 40% до 90%, а игровой FPS только с 40% до 47%, при этом потребление и нестабильность растут с 25% до 100%. Типичный стабильный максимум — 38 CU; 40 CU это лотерея, нужны более 300 Вт и водяное охлаждение.">
</p>
...
<p align="center">
  <img src="assets/diagrams/idle-power-tradeoff.svg" width="100%" alt="Потребление в простое: потребление GPU падает с 80 Вт до 5 Вт после governor, андервольта и автосна, но общее потребление от розетки снижается лишь со 125 Вт до 110 Вт, и только автосон роняет его до 10 Вт.">
</p>

Governor и андервольт срезают GPU, но процессор и GDDR6 продолжают есть столько же. Пока не включится автосон, розетка разницы почти не замечает. У плат с высокой утечкой пол — около 70–80 Вт.

</details>

<sub>Исходные данные: [`assets/diagrams/data.json`](assets/diagrams/data.json) · пересборка: `node assets/diagrams/build.mjs`</sub>
